...
static void print_ping_description(const ping_result_t *result, const flags_t *flags) {
  const char *description = get_icmp_description(result);
  ...
  if (flags->options.verbose) {
    ...
    for (uint32_t j = 0; j < sizeof(*result->ip_icmp); ++j)
      printf("%02x%s",
             *((unsigned char *)result->ip_icmp + j),
             (j % 2) ? " " : ""); /* Group bytes two by two.  */
    printf("\n");
    printf("Vr HL TOS  Len   ID Flg  off TTL Pro  cks      Src\tDst\tData\n");
    printf(
        " %1x  %1x  %02x", result->ip_icmp->ip_v, result->ip_icmp->ip_hl, result->ip_icmp->ip_tos);
    printf(" %04x %04x",
           (result->ip_icmp->ip_len > 0x2000) ? ntohs(result->ip_icmp->ip_len)
                                              : result->ip_icmp->ip_len,
           ntohs(result->ip_icmp->ip_id));
    printf("   %1x %04x",
           (ntohs(result->ip_icmp->ip_off) & 0xe000) >> 13,
           ntohs(result->ip_icmp->ip_off) & 0x1fff);
    printf("  %02x  %02x %04x",
           result->ip_icmp->ip_ttl,
           result->ip_icmp->ip_p,
           ntohs(result->ip_icmp->ip_sum));
    ...
    uint8_t *cp =
        (unsigned char *)result->ip_icmp + sizeof(*result->ip_icmp); /* point to options */
    ...
    printf("ICMP: type %u, code %u, size %u", type, code, ntohs(result->ip_icmp->ip_len) - hlen);
...
static char *get_icmp_description(const ping_result_t *result) {
  for (uint32_t i = 0; i < sizeof(icmp_code_description) / sizeof(icmp_code_description[0]); i++) {
    if (result->status == PING_SUCCESS && result->type == icmp_code_description[i].type &&
        result->code == icmp_code_description[i].code) {
